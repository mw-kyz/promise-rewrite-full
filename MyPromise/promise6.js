...
class MyPromise6 {
  ...
  then (onFulfilled, onRejected) {
    ...
    //   throw reason
    // })
    let promise2 = new MyPromise6((resolve, reject) => {
    ...
    return this.then((value) => {
      // 上一个是成功，回调里面是成功，后面的then走成功，把上一个成功的结果传递下去，符合预期
      // 上一个是成功，回调里面是失败，后面then的成功回调不执行，回调里的失败会透传下去，下一个then自然会走回调里的失败，符合预期
      return MyPromise6.resolve(finallyCallBack()).then(() => value)
    }, (reason) => {
      // 上一个是失败，回调里面是成功，后面的then会走成功，把上一个失败的结果传递下去，符合预期
      // 上一个是失败，回调里面是失败，后面的then的成功回调不执行，回调里的失败会透传下去，下一个then自然会走回调里的失败，符合预期
      return MyPromise6.resolve(finallyCallBack()).then(() => {
        throw reason
      })
    })
  }
